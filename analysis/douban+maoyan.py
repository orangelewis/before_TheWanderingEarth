...
if __name__ == '__main__':
    ...
    for item_d in douban_list1:
        ...
        for item_m in maoyan_list1:
            if item_m['name'] ==item_d['name']:
                gap['gap'] = item_m["rank"] + item_d["rank"]
                gap['rm'] = item_m["rank"]

        gap_list.append(gap)

        gap_list = sorted(gap_list, key=lambda item: item["gap"])
    ...
    bar = Bar("2011-2018年科幻电影豆瓣猫眼综合排名TOP25", title_pos='center', title_top='18', width=1400, height=800)
    ...
    bar.add("猫眼", attr, v3, is_convert=True, xaxis_min=0, xaxis_max=35, yaxis_label_textsize=25,
            is_yaxis_boundarygap=True, yaxis_interval=0, is_label_show=True,  label_pos='right',
            is_yaxis_inverse=True, is_splitline_show=False, is_stack=True)

    bar.render("2011-2018年科幻电影豆瓣猫眼综合TOP25.html")
